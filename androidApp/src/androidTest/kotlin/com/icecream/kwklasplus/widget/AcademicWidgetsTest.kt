...
class AcademicWidgetsTest {
    ...
    @Test fun expandedTimetableShowsScrollableWeekendSectionOnlyWhenNeeded() {
        instrumentation.runOnMainSync {
            for (entries in listOf(emptyList(), listOf(TimetableEntry("일요일 수업", 6, "9:0", "10:0", "301", "S")))) {
                ...
                val root = AcademicWidgets.view(context, 9876, AcademicWidgetKind.TIMETABLE, 350, 400, data)
    ...
    @Test fun remoteViewsInflateAcrossEveryRequestedSizeAndRenderMonthlyBars() {
        ...
        instrumentation.runOnMainSync {
            for (kind in AcademicWidgetKind.entries) {
                for ((width, height) in listOf(70 to 70, 140 to 70, 140 to 140, 280 to 140, 350 to 180, 280 to 250, 350 to 400)) {
                    ...
                    val root = AcademicWidgets.view(context, 9876, kind, width, height, data).apply(context, FrameLayout(context))
                    ...
                    val full = AcademicWidgetPolicy.layout(width, height) == AcademicWidgetLayout.FULL
                    if (full) assertEquals(View.VISIBLE, root.findViewById<View>(R.id.academic_widget_grid).visibility)
                        ...
                        val focus = kind == AcademicWidgetKind.TIMETABLE && AcademicWidgetPolicy.layout(width, height) == AcademicWidgetLayout.COMPACT
                    ...
                    File(context.getExternalFilesDir(null), "widget-${kind.name}-$width-$height.png").outputStream().use {
    ...
    @Test fun missingDataDoesNotContainGridOrSummary() {
        instrumentation.runOnMainSync {
            ...
            val root = AcademicWidgets.view(context, 9877, AcademicWidgetKind.CALENDAR, 280, 280, data)
                .apply(context, FrameLayout(context))
