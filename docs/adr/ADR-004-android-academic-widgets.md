...
`AtomicFile` 읽기·쓰기·삭제는 순서를 보장하는 단일 직렬화 경로에서 IO dispatcher로 실행한다. 앱 시작 시 파일을 비동기로 읽은 뒤 메모리 스냅샷을 준비하고 위젯을 다시 그린다. 표시 경로는 메모리 스냅샷만 읽으며, 로그아웃 삭제는 진행 중인 쓰기 뒤에 실행하고 이전 계정의 대기 중인 쓰기는 거부한다.
