...
| **딥링크 스키마** | `kwklasplus://library-qr` (출입증 열기). 설정 전용 딥링크는 두지 않으며, 설정은 웹 브리지(`openLibraryQRSettingsModal`)와 앱 내 QR 시트의 설정 버튼으로만 연다 | `iosApp` URL Types 등록 및 라우팅 |
...
| 기술 후보군 | 동작 방식 | 채택 여부 | 채택 / 기각 이유 |
|---|---|---|---|
| **도서관 QR 시트 단독 잠금 예외 허용 (Android F-022 패리티)** | 앱 잠금이 활성화되어 있어도 `library-qr` 딥링크는 인증 없이 즉시 QR 시트 팝업 | **채택** | 1. **게이트 신속 통과**: 게이트 앞에서 Face ID/PIN 입력 지연 없이 0.1초 만에 바코드 인식 가능.<br>2. **안전한 경계 격리**: QR 시트만 단독으로 열리며, 시트를 닫거나 타 화면으로 이동 시에는 앱 전체 잠금이 유지됨.<br>3. **미설정 보호**: 도서관 출입증이 미설정 상태인 경우 인증 없이 설정 화면을 열지 않고, 설정 안내 후 앱 잠금 해제를 거쳐 설정 화면으로 이동한다. |
...
- `kwklasplus://library-qr`:
  ...
  - 도서관 자격증명이 미설정된 경우: "먼저 앱에서 모바일 학생증 설정을 완료해주세요" 알림 표시 후, **앱 잠금 인증을 거쳐 설정 화면(LibraryQRSettingsSheet)으로 이동**한다.
- 설정 전용 딥링크는 두지 않는다. 출입증 설정은 웹 브리지(`openLibraryQRSettingsModal`)와 앱 내 QR 시트의 설정 버튼으로만 연다.
...
- **KLAS 로그아웃 시**:
  ...
  - 위젯 탭 시 미로그인 상태이면 설정 시트를 열지 않고 설정 안내 알림만 표시한 후 시작 화면 유지.
- **네트워크 오류 / 도서관 서버 장애 / 정보 불일치**:
  - 진입 시 안드로이드와 동일하게 `clearCache` 후 1회 자동 재시도.
  - 재시도 실패 시 시트를 닫지 않고 오류 UI(안내 메시지 및 「출입증 재설정」 버튼)를 표시하고 위젯 진입 시 Toast 노출.
  - 위젯 진입 오류 화면에서 「출입증 재설정」 탭 시 앱 잠금이 활성화되어 있다면 잠금 해제를 거쳐 설정 시트로 이동.
  - 최종 실패 상태에서는 30초 자동 갱신 타이머를 중단하며(화면 밝기는 시트가 닫힐 때 복원), 새로고침 성공 시에만 타이머를 재개.
