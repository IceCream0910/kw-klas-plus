...
| F-033 | 학사 시간표·캘린더 홈 위젯 | Android provider 4종 + 공통 스냅샷 | WidgetKit 캘린더 S/M/L + 주간 시간표 Large | P1 | Implemented (Android 실기기 검증 대기) | Android: 축소형 수업/일정 목록·실시간 수업 상태, 확장형 평일 그리드/월간 달력, 주말 과목 목록, 수동/정기 일정 갱신, 잠금 진입 및 탭 딥링크. 기존 provider는 축소형으로 유지. 샘플 미리보기 4종, 실기기 런처 검증 대기. iOS: 캘린더 agenda/monthBars, 시간표 Large 그리드만. iOS 17+ 캘린더 새로고침은 AppIntent로 KLAS 조회·만료 시 저장 자격증명 재인증([ADR-006](../adr/ADR-006-ios-widget-interactive-refresh.md)). iOS 16은 앱 열기. 승인 차이: 최초 추가 시 확장 네트워크 없음(`앱을 열어 가져오기`), SESSION·암호화 비밀번호는 Keychain Access Group에만 저장, 축소형 남은 시간/진행률 없음, WidgetKit 갱신 예산, 로그아웃 직후 timeline 잔상, scheme `kwklasplus`↔`klasplus` 매핑. 도서관 QR `systemSmall` 유지. iOS 16 앱 열기·iOS 17 앱 미실행 새로고침·잠금 전 첫 부팅·저전력/오프라인·계정 전환은 실기기 검증 대기. [ADR-004](../adr/ADR-004-android-academic-widgets.md), [ADR-005](../adr/ADR-005-ios-academic-widgets.md) |
