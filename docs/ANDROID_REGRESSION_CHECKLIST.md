...
| F-009 캘린더 | 시작/종료 날짜·시간 선택 및 취소, 제스처/3버튼 모드 Web BottomSheet + IME | timezone/값 순서/취소 처리 정상, 취소/확인 버튼이 키보드에 가리지 않음 | D-02 | 재검증 필요 |
| F-010 프로필/학생증 | QR 요청, modal 열기·닫기, 밝기 복원 | 학생증·도서관 QR 표시, 민감 값 외부 노출 없음 | D-02 | 통과 |
| F-011 성적 등 | 성적/석차/장학/KLAS AI 진입, 성적 상세 시트 | SESSION callback, 시트 콘텐츠 표시, 학교/외부 링크 분기 정상 | D-02 | 재검증 필요 |
...
| F-020 도서관 QR | 최초 설정, 입력란별 IME 표시, 작은 화면 스크롤, 성공, 잘못된 정보, 캐시 만료, 재설정 | IME 표시 중 키보드 위로 리사이즈된 영역에서 시트가 `MATCH_PARENT` 높이를 사용하고 저장 버튼은 최하단 고정, 입력 영역만 스크롤, repository 오류 구분, QR/밝기/캐시 정상 | D-01,D-02 | 재검증 필요 |
